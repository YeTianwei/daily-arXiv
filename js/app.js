...
function showPaperDetails(paper, paperIndex) {
  ...
  const chatgptPrompt = `请你阅读这篇文章${paper.url.replace('abs', 'pdf')}，总结一下这篇文章解决的问题、相关工作、研究方法、做了什么实验及其结果、结论，最后整体总结一下这篇文章的内容。请用专业、清晰的中文回答；当涉及分点回答时，用 Markdown 格式输出。`;
  document.getElementById('chatgptLink').href = `https://chatgpt.com/?q=${encodeURIComponent(chatgptPrompt)}`;
